...
var compile = function(sel) {
  ...
  while (sel) {
    ...
    if (sel[0] === '!') {
      sel = sel.substring(1);
      subject = { qname: qname };
      buff.push(selectors.subject(subject));
    }
  ...
  if (subject) {
    filter = makeSubject(filter, subject, qname);
    filter.qname = subject.qname; // '*'
    filter.sel = sel;
    return filter;
  }
...
var makeSubject = function(func, subject, qname) {
  var test = makeTest(func);
  return function(el) {
    var node = el.ownerDocument
