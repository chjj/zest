...
// TODO
// - Add the reference combinator to the combinator rule.
...
var compile = function(sel) {
  ...
  while (sel) {
    ...
    if (sel[0] === '!') {
      sel = sel.substring(1);
      subject = makeSubject(filter);
      subject.qname = qname;
      buff.push(subject.set);
    }
    ...
    if (cap = rules.ref.exec(sel)) {
      ...
      //
      //   var attr = el.getAttribute(ref.name) || '';
      //   if (attr[0] === '#') attr = attr.substring(1);
      //
      //   return el === ref.node.id; // ref.node would be the label e.g.
      // });
      // filter.push(combinators.noop(makeSimple(buff)));

      ref = combinators.ref(makeSimple(buff), cap[1]);
      filter.push(function(el) {
        if (!ref.nodes || !ref.nodes.length) return;
        return ref(el, ref.nodes.pop());
      });
      buff = [];
      continue;
    }
  ...
  if (ref) {
    var filter_ = filter;
    filter = function(el) {
      ref.nodes = zest('*', el.ownerDocument);
      while (ref.nodes.length) {
        // ref.node = ref.nodes.pop();
        if (filter_(el)) {
          ref.nodes = null;
          return true;
        }
    ...
    filter.qname = filter_.qname;
    filter.lname = filter_.lname;
    filter.sel = filter_.sel;
  }
...
var makeSubject = function(filter) {
  var test = makeTest(filter)
    , target;
