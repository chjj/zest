...
// - Recognize (both?) subject selector while parsing.
...
select = (function() {
  ...
  return function(sel) {
    if (~sel.indexOf(' ')) return _select(sel);
    try {
      if (sel[0] === '#' && /^#\w+$/.test(sel)) {
        return [context.getElementById(sel.substring(1))];
      }
      if (sel[0] === '.' && /^\.\w+$/.test(sel)) {
        sel = context.getElementsByClassName(sel.substring(1));
