...
/**
 * Handle :column pseudo-classes
 */
  function calc_cols (td, cache) {
    while (td = td.parentNode) if (td.nodeName === 'TABLE') break;
    
    var cols = td.getElementsByTagName('col')
    var col_list = []
      , col
      , col_pos = 0
      , td_pos = 1
    ...
    for (; col = cols[col_pos]; col_pos++) {
      spans = parseInt(col.getAttribute('span')) || 1
      
      td_pos += spans
      
      while (spans) {
        col_list[td_pos - spans] = col
        spans--
      }
    }
...
var selectors = {
  ...
  ':column': function(selector) {
    var test = zest.compile(selector)
    
    return function(el) {
      if (el.parentNode.nodeType !== 1 || el.nodeName !== 'TD') return;
      ...
      while (rel) {
        spans = parseInt(rel.getAttribute('span')) || 1
        if (rel.nodeName === 'TD') pos += spans
        if (rel === el) {
          while (spans--) {
            temp_pos = pos - spans
            if (cols[temp_pos] && test(cols[temp_pos])) return true
          }
          return false;
        }
        rel = next(rel);
  ...
  ':nth-column': function(param) {
    return selectors[':column'](':nth-child(' + param + ')')
  },
  ':nth-last-column': function(param) {
    return selectors[':column'](':nth-last-child(' + param + ')')
  },
  ':current': function() {
