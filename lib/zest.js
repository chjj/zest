...
var makeInside = function(endChar) {
  var inside = /(?:"(?:\\"|[^"])*"|'(?:\\'|[^'])*'|\\["'_]|[^"'_])+/;
  var ins = inside.source.replace(/_/g, endChar);
...
var rules = {
  qname: /^(\w+|\*)\s*/,
  simple: /^([.#][\w\-]+|:[\w\-]+(?:\(inside\))?|\[inside\])\s*/,
  // only real combinators
  //combinator: /^([ >~+])\s*/
  // any combinator
  combinator: /^([^\w\])])\s*/
  // allow "nothing" to be a space
  // could instead us a regex inside the loop
  // after a combinator is not found: if (/\s*\S/.test(sel)) op = ' ';
  //combinator: /^([^\w\])]|(?=\s*[^\s]))\s*/
};
...
var compile = function(sel) {
  ...
  while (sel) {
    ...
    if (cap = rules.combinator.exec(sel)) {
      ...
      //console.log('OP: %s', JSON.stringify(op));
    ...
    //test
    // console.log(sel ? buff.concat(op) : buff);
...
var paren = /^(:[\w-]+)\((pvalue)\)/
paren = replace(paren, 'pvalue', makeInside(')'));
