  ...
  'ref': function(test, name) {
    return function(el) {
      var node = el.ownerDocument
        , e = node.getElementsByTagName('*')
        , i = e.length
        , attr;

      while (i--) {
        if (!e[i].getAttribute || e[i].nodeType !== 1) continue;
        attr = e[i].getAttribute(name);
        if (!attr) continue;
        if (attr[0] === '#') attr = attr.substring(1);
        if (attr === el.id && test(e[i])) return e[i];
      }
    };
  }
};
...
var compile = function(sel) {
  ...
  while (sel) {
    ...
    if (cap = rules.ref.exec(sel)) {
      sel = sel.substring(cap[0].length);
      filter.push(combinators.ref(makeSimple(buff), cap[1]));
      buff = [];
      continue;
    }
