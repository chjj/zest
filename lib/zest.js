...
var parse = function(sel) {
  ...
  return sel[0].name === '*'
    ? selectors['*']
    : selectors.type(sel[0].name);
};

var parseString = function(sel) {
  switch (sel.type) {
    case 'class':
      return selectors.attr('class', '~=', sel.name);
    case 'id':
      return selectors.attr('id', '=', sel.name);
    case 'attr':
      return selectors.attr(sel.name, sel.op, sel.val);
    case 'pseudo':
      return sel.val
        ? selectors[sel.name](sel.val)
        : selectors[sel.name];
    case 'qname':
      return sel.name === '*'
        ? selectors['*']
        : selectors.type(sel.name);
  }
};
...
var tok = function(cap) {
  var tok = {};
  if (cap[1]) {
    // class/id
    tok.type = cap[1][0] === '.' ? 'class' : 'id';
    tok.name = cap[1].substring(1);
  } else if (cap[2]) {
    // pseudo-name
    tok.type = 'pseudo';
    tok.name = cap[2];
...
var compile = function(sel) {
  ...
  while (sel) {
    ...
    if (cap = rules.qname.exec(sel)) {
      sel = sel.substring(cap[0].length);
      qname = cap[1];
      buff.push({type:'qname',name:qname});
