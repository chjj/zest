...
var child = function(el, last) {
  if (el = last ? el.lastChild : el.firstChild) {
    while (el.nodeType !== 1
           && (el = last ? el.previousSibling : el.nextSibling));
  }
  return el;
};
...
var nth = function(param, test, last) {
  ...
  return function(el) {
    ...
    var rel = child(el.parentNode, last)
      , pos = 0
      , diff;
...
var combinators = {
  ...
  '//': function(test, name) {
    return function(el) {
      var e = document.getElementsByTagName('*') // cant recurse zest
        , i = e.length
        , attr;
...
var select = function(sel, context) {
  ...
  if (subject) {
    while (el = scope[i++]) {
      if (test(el) && !~indexOf.call(results, subject))
